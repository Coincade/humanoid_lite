...
from humanoid_lite_lowlevel.policy.config import Cfg
from humanoid_lite_lowlevel.policy.gamepad import Se2Gamepad
...
class MujocoEnv:
    def __init__(self, cfg: Cfg):
        ...
        import os
        mjcf_dir = "source/humanoid_lite_assets/data/mjcf"
        
        # Change to MJCF directory to resolve relative paths correctly
        original_cwd = os.getcwd()
        os.chdir(mjcf_dir)
        
        try:
            if cfg.num_joints == 20:
                self.mj_model = mujoco.MjModel.from_xml_path("hl_scene.xml")
            else:
                self.mj_model = mujoco.MjModel.from_xml_path("hl_biped_scene.xml")
        finally:
...
class MujocoVisualizer(MujocoEnv):
    """MuJoCo simulation environment for the Humanoid Lite robot.
